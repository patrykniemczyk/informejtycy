...
class DockerManager():
    ...
    def build_for_checker(self, executable_file_name: str) -> tuple[str, bytes]:
        ...
        try:
            stdout = subprocess.check_output(
                ["docker", "build", "-t", self.execution_image_name, self.compiled_dir], stderr=subprocess.STDOUT)
    ...
    def run_for_checker(self, input_: bytes, memory_limit_MB: int, timeout: int) -> tuple[str, bytes]:
        ...
        process = subprocess.Popen(["docker", "run", "--rm", "--read-only", "--cap-drop=ALL", "--memory-swap=256m", "--network", "none", "-m",
                                   f"{memory_limit_MB}m", f"--cpus={DOCKER_CPU_LIMIT}", "--name", container_name, "-i", self.execution_image_name, "/app/a.out"], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
    ...
    def clear_images(self) -> tuple[str, bytes]:
        ...
        try:
            stdout = subprocess.check_output(
                ["docker", "system", "prune"], input='y'.encode('utf-8'))
            status = DckStatus.success
        except Exception as e:
            status = DckStatus.server_error
