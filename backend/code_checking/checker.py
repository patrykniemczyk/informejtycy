...
class Checker:
    ...
    def __init__(self, logger: Logger, compiler: Compiler, pack_loader: PackLoader):
        ...
        self.check_queue: list[tuple[str, int, str,
                                     Callable[[CheckResult, str], None]]] = []
    ...
    def listen(self) -> None:
        ...
        while True:
            if len(self.check_queue) > 0:
                ...
                self.logger.debug(
                    f"docker system prune: {status}", self.listen)
    ...
    def check(self, code_file: str, ex_id: int) -> CheckResult:
        ...
        for test_in, test_out in test_pack:
            status, output = self.docker_manager.run_for_checker(
                input_=test_in, memory_limit_MB=pack_config['memory_limit'], timeout=pack_config['time_limit'])
