...
class Compiler:
    ...
    def compile(self, filename: str) -> str:
        ...
        command = [self.compiler, join(self.input_dir, filename), "-O2",
                   "--std=c++23", "-o", join(self.output_dir, target_filename)]
        ...
        try:
            ...
            self.logger.alert(
                f"{self.compiler} compiler is not installed!", self.compile)
        except:
            pass
