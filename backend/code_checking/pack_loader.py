...
class PackLoader:
    ...
    def load_bytes(self, index: int) -> list[tuple[bytes, bytes]]:
        ...
        if index >= self.get_pack_count():
            ...
            self.logger.alert(
                f"Given pack index {index} doesn't exists", self.load_bytes)
            return [(b"", b"")]
        ...
        with zipfile.ZipFile(os.path.join(self.pack_dir_path, self.pack_files[index])) as pack:
            for i in range(len(pack.filelist) // 2):
                try:
                    ...
                    if self.logger is None:
                        raise Exception(
                            "Number of input files must match the number of output files.")
                    self.logger.alert(
                        "Number of input files must match the number of output files.", self.load_bytes)
                    tests.append((b"", b""))
                except Exception as e:
                    if self.logger is None:
                        raise Exception(
                            f"Challenge Complete! How Did We Get Here? | {e.__class__.__name__}: {e}")
                    self.logger.error(
                        f"Challenge Complete! How Did We Get Here? | {e.__class__.__name__}: {e}", self.load_bytes)
                    tests.append((b"", b""))
    ...
    def load_config(self, index: int) -> dict[str, int]:
        ...
        if index >= self.get_pack_count():
            if self.logger is None:
                raise Exception(f"Given pack index {index} doesn't exists")
            self.logger.alert(
                f"Given pack index {index} doesn't exists", self.load_config)
            return conf
        ...
        with zipfile.ZipFile(os.path.join(self.pack_dir_path, self.pack_files[index])) as pack:
            try:
                ...
                self.logger.alert(
                    "Config file is not present.", self.load_config)
            except ValueError:
                if self.logger is None:
                    raise Exception("Time or memory limit is not an integer.")
                self.logger.alert(
                    "Time or memory limit is not an integer.", self.load_config)
            except Exception as e:
                if self.logger is None:
                    raise Exception(
                        f"Challenge Complete! How Did We Get Here? | {e.__class__.__name__}: {e}")
                self.logger.error(
                    f"Challenge Complete! How Did We Get Here? | {e.__class__.__name__}: {e}", self.load_config)
