...
class Logger:
    ...
    def warn(self, message: str, from_: Callable[[Any, ...], Any]) -> None:
        self._log_any(message, from_, "WARNING",
                      Color.WARNING, Color.WARNING_ID)
    ...
    def _log_any(self, message: str, from_: Callable[[Any, ...], Any], type_: str, color: str, id_: int) -> None:
        if "__self__" in from_.__dir__():
            self.logs.append(
                f"{Color.FROM}FROM: {from_.__self__.__class__.__name__}.{from_.__name__} {color}{type_}:{Color.NORMAL} {message}")
        else:
            self.logs.append(
                f"{Color.FROM}FROM: {from_.__name__ if from_.__name__ != 'main' else 'app.main'} {color}{type_}:{Color.NORMAL} {message}")
