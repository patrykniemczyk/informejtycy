...
def print_code_result(result: CheckResult, auth: str) -> None:
    with results_lock:
        ...
        logger.spam(f"{len(results)} submissions are waiting",
                    print_code_result)
        ...
        with results_lock:
            for res in dict(results):  # dict(...) to make copy
                if time.time() - results[res][1] >= RECEIVE_SUBMISSION_TIME:
                    results.pop(res)
                    logger.spam(
                        f"Cleaning, left {len(results)} submissions", clean_results)
...
with app.app_context():
    ...
    lt.start()
    # Listens for cleaning results from dictionary, which are on /status/<auth>
...
def get_task_results(auth: str) -> tuple[str, int]:
    res: tuple[str, int] = results.get(
        auth, (UnauthorizedCheckResult().as_dict(), 0))
