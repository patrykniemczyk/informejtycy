...
def main() -> int:
    global pack_loader
    pack_loader = PackLoader(Logger(), "../tests",
                             ".test", "in", "out", "CONFIG")
    ...
    try:
        ...
        if not test_all in 'tn':
            print(
                "Toż Ci mówie ~~idioto~~, że 't' lub 'n' co mie tu jakieś głupoty wypisujesz")
        ...
        file_min_name = input(
            "Podaj nazwę pliku, od którego należy zacząć sprawdzanie poprawności: ")
        file_max_name = input(
            "Podaj nazwę pliku, na którym nalezy skończyć sprawdzanie poprawności: ")

        check(all_tests.index(file_min_name),
              all_tests.index(file_max_name)+1, all_tests)
    except ValueError:
